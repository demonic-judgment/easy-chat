...
#### 🏷️ 强大的提示词模板结构化控制
- 标签化模板语法：`*role_description*`、`*chat_history*`、`*user_reply*`、`*自定义*`
- 自由组合模板片段，构建复杂的提示词结构
- 支持指定模板适用的智能体，实现精细化控制
- 自定义变量灵活注入，满足多样化场景需求

#### 🖼️ 悬浮图片 - 不一样的聊天视觉效果
- **突破传统头像限制** - 图片不再局限于角落的小头像，可自由拖拽、调整大小
- **实时预览** - 支持拖拽到任意位置，缩略图快速加载
- **灵活控制** - 可随时显示/隐藏，不打断对话节奏
- **多图管理** - 支持同时悬浮多张图片，工作流更高效
