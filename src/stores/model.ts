...
export const useModelStore = defineStore('model', () => {
  ...
  const createModel = async (data: Omit<ModelConfig, 'id' | 'createdAt' | 'updatedAt'>): Promise<ModelConfig> => {
    const now = Date.now()
    ...
    await db.models.put(toStorable(model))
    models.value.push(model)
    return model
  }

  const updateModel = async (id: string, data: Partial<Omit<ModelConfig, 'id' | 'createdAt' | 'updatedAt'>>) => {
    ...
    if (existing) {
      ...
      await db.models.put(toStorable(updated))
      const index = models.value.findIndex(m => m.id === id)
      if (index !== -1) {
        models.value.splice(index, 1, updated)
        return true
      }
    }
    return false
